  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "Concurrent rendering changes how Fiber's workLoop operates. In sync mode, it runs without stopping. In concurrent mode, it yields to the browser every 5ms using MessageChannel. The scheduler uses priority lanes (SyncLane, InputContinuousLane, DefaultLane, TransitionLane) based on update origin. Higher-priority updates interrupt lower-priority renders. This works because the render phase is pure and builds a work-in-progress tree without touching the DOM. Abandoning a partial render has no side effects. The commit phase stays synchronous. Components may render but never commit, making render-phase side effects dangerous. StrictMode double-renders to catch this. The lanes model replaces expiration times and supports batching same-priority updates."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "startTransition assigns TransitionLane priority instead of SyncLane. The scheduler yields every 5ms via shouldYield(). If a higher-priority update arrives, the transition render is abandoned and restarted from scratch with the latest state (not resumed, to avoid inconsistent state). Components in the transition subtree may render many times without committing. isPending flips to true at start and false on commit. Rapid successive transitions are batched so only the latest one commits (transition entanglement). The standalone startTransition (from 'react') works outside components but has no isPending."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "useDeferredValue schedules two renders internally. The urgent render returns the previous value, letting memoized components bail out. The transition render returns the new value at TransitionLane priority and is interruptible. The fiber stores both current and pending values. For this to help, the consumer must be wrapped in React.memo so it skips the urgent pass. In React 19, useDeferredValue accepts an initialValue for SSR: `useDeferredValue(value, initialValue)`. The initial render uses initialValue and schedules a transition with the real value, useful for deferring heavy renders during hydration."
      },
  ...
  return <div dangerouslySetInnerHTML={{ __html: html }} />;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Suspense uses the same 'throw a thenable' mechanism as React.lazy. When use(promise) is called and the promise is pending, it throws. The Fiber reconciler catches it in throwException, switches the Suspense fiber to fallback, and registers an onResolve callback. On resolve, React schedules a re-render (ping) at transition lane. The retried component gets the resolved value. The Promise must be referentially stable across renders. Creating `use(fetch('/api'))` inside render makes a new Promise each time, causing infinite suspension. This is why you need framework caching (React cache(), RSC, route loaders). Unlike other hooks, use() can be called conditionally and in loops since it reads from an external resource. With transitions, Suspense keeps old UI visible instead of showing fallbacks, preventing jarring loading states."
      },
