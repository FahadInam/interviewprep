  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "Context uses a linked list of providers on the fiber tree. Each context has a _currentValue property. When a Provider gets a new value, React walks the fiber subtree to find all fibers that depend on that context (via fiber.dependencies). Each match gets marked for re-render, bypassing React.memo and shouldComponentUpdate. Value comparison uses Object.is(). A new object reference (even with same content) re-renders ALL consumers. This is the biggest Context performance trap."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Context propagation marks ALL consumer fibers as dirty when ANY part of the value changes. Object.is(oldValue, newValue) checks the Provider's value prop. A new reference means ALL consumers get scheduled for re-render. There is no built-in selector. React RFC #119 proposed useContextSelector but was never merged. The team recommends external libraries or useSyncExternalStore. Split contexts work because each context propagates independently. Alternative: store state in a ref and use useSyncExternalStore with subscriptions for per-consumer selector-based re-rendering."
      },
