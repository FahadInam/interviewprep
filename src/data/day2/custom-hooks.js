  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "React tracks hooks by their call order — first useState is hook #1, second is hook #2, etc. On every re-render, React walks through them in the same order. If you put a hook inside an if-statement and it gets skipped, every hook after it reads the wrong data — hook #3 gets hook #2's state. This is why the lint rule exists: it's not a suggestion, it's preventing data corruption. The workaround for conditional logic is always the same: call the hook unconditionally, put the condition INSIDE the hook's callback. The React Compiler (currently in beta) may eventually auto-generate correct code, but until then, follow the rules strictly."
      },
  ...
  return <input value={value} onChange={e => setValue(e.target.value)} />;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Custom hooks are a naming convention, not a special React feature. When ComponentA calls useMyHook() which calls useState(), that state belongs to ComponentA, not some shared location. There is no 'hook scope' — hooks inside a custom hook run in the calling component's context. This is why each component using the same custom hook gets independent state. The 'use' prefix matters for two reasons: (1) ESLint only enforces hook rules for functions starting with 'use', and (2) React verifies hooks run inside a valid render context. Custom hooks can nest to any depth — all hooks flatten onto the calling component's internal list, which is why call order must be consistent."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Hook composition works because all hooks in a custom hook share the same render cycle and batching. React 18's automatic batching means multiple setState calls across composed hooks merge into one re-render. For testing, renderHook from @testing-library/react creates a minimal test component, giving you result.current and act() for triggering updates. Async effects need waitFor or act(async () => ...). Use jest.useFakeTimers for debounce/throttle tests. Integration tests should verify that composed hooks interact correctly — for example, that changing the debounced URL triggers an abort in the fetch hook."
      },
