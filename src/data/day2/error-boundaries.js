  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "In React's Fiber architecture, error boundaries work across two render phases. When a fiber throws, React walks up the fiber tree to find the nearest boundary (via throwException in ReactFiberThrow). It unwinds the tree to that boundary and replays the render with fallback UI. getDerivedStateFromError runs in the render phase (synchronous, pure). componentDidCatch runs in the commit phase (side effects allowed). useEffect errors are caught differently: React wraps effect execution in try-catch and reports to the nearest boundary. A boundary cannot catch its own errors because there is no mechanism to re-enter the same boundary when its render throws. In concurrent mode, React may retry rendering once before committing the fallback."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Error boundaries create recovery points in the fiber tree. When an error occurs, React unwinds fibers up to the boundary and replaces the subtree with the fallback. All state in the subtree is lost. Changing the boundary's key prop (`<ErrorBoundary key={retryCount}>`) creates a new fiber subtree, cleanly resetting all children. For Suspense interaction: if a lazy component fails to load, the error goes to the error boundary, not Suspense. Use ErrorBoundary outside Suspense: `<ErrorBoundary><Suspense fallback={<Spinner />}><LazyComponent /></Suspense></ErrorBoundary>`. In concurrent mode, React may re-render boundary children once before committing the fallback. componentDidCatch provides the componentStack showing the exact component hierarchy, which is more useful than a regular JS stack trace."
      },
