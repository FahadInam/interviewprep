  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "Fiber is both an architecture and a data structure. Each Fiber node is a JS object tied to a component or DOM element. It holds the component type, props, state, effect list, and tree pointers. The core idea is replacing the JS call stack with a 'virtual stack' — each Fiber is a stack frame React controls. The work loop (workLoopConcurrent) calls performUnitOfWork on one Fiber at a time, then checks shouldYield() (usually a 5ms deadline). If time is up, it yields via MessageChannel. beginWork goes down into children, completeWork bubbles up building effects. The old recursive approach could not be paused because you cannot freeze a JS call stack mid-execution. Fiber's linked list (child -> sibling -> return) is easy to resume — the current position is just a pointer."
      },
  ...
  return COMPLETE;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "The Fiber tree is actually two trees: 'current' (on screen) and 'workInProgress' (being built). Each Fiber has an 'alternate' pointer to its twin — this is double buffering. During render, React clones current fibers into WIP fibers, modifying the copies. On commit, the WIP tree becomes current (a pointer swap on fiberRoot.current). The traversal: beginWork(current, wip, lanes) diffs children and returns the first child (or null at leaves). completeWork(current, wip, lanes) creates/updates DOM nodes and bubbles effect flags up via subtreeFlags. Full path: beginWork goes down via child pointers. At a leaf, completeWork runs, then move to sibling. No more siblings? completeWork on parent, then its sibling. Continue until root. This mimics recursive DFS but with explicit pointers instead of a call stack."
      },
...
// and continue later. This is how concurrent rendering works.`
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Lanes are bitmasks replacing the older expiration times model. Key lanes: SyncLane (0b0000001 — clicks, key presses), InputContinuousLane (0b0000100 — mousemove), DefaultLane (0b0010000 — normal setState), TransitionLanes (16 lanes for useTransition), and IdleLane for background work. When setState is called, React picks a lane based on context (startTransition gets TransitionLane). ensureRootIsScheduled picks the highest pending lane: SyncLane uses microtask scheduling, others use the Scheduler. During render, only updates matching renderLanes are processed. If a higher-lane update arrives, shouldYield() triggers, and React drops the current WIP tree to restart with merged lanes. The commit phase walks fibers with effect flags, applies DOM mutations (insertBefore, removeChild, property updates), then swaps trees (fiberRoot.current = finishedWork)."
      },
