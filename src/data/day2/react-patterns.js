  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "A controlled input without onChange is read-only — React overwrites the DOM value on every render, so user typing gets immediately reverted. Subtle bug: setting state asynchronously in onChange (e.g., inside a debounce) can cause cursor jumps because React resets the value before the update lands. For forms with 50+ fields, controlled inputs can feel sluggish because every keystroke re-renders the parent — use React Hook Form (uncontrolled + refs) or isolate each field in its own component. File inputs MUST be uncontrolled because JavaScript can't programmatically set their value for security reasons. The decision framework: controlled when you need instant validation or formatting, uncontrolled when you only need the value on submit."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Lifting state is a natural result of React's one-way data flow, but the trade-off is performance: every update re-renders the parent and all children receiving that state. The key mistake is over-lifting — moving state too high creates unnecessarily large re-render trees. Three fixes: (1) keep state as close to its consumers as possible, (2) use React.memo on children that receive the lifted state, (3) for deeply nested consumers, use Context or an external store with targeted subscriptions (Zustand, Jotai) instead of prop drilling. A senior's instinct: if lifting state more than 2-3 levels, it's a signal to reach for a different pattern."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Two implementation approaches: React.cloneElement or Context. cloneElement injects props into direct children — simple but breaks with fragments, wrappers, or deeply nested children. Context is more flexible since children consume it at any depth. Key performance consideration: stabilize the context value with useMemo, otherwise every parent render creates a new object and re-renders all consumers. For frequently updating compound components, split the context (one for stable methods, one for changing values) or use a ref-based registry where children register via useEffect. The decision: use cloneElement for simple, shallow compound components; use Context for anything deeper or more flexible."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Render props were the main code reuse pattern before hooks. They avoid HOC issues like wrapper hell and prop collisions, but introduce their own problem: nesting multiple render prop components creates callback hell. Performance-wise, inline render functions create new references on every parent render, preventing React.memo bailouts. Hooks replaced most render prop use cases. Render props are still the right choice for component-level inversion of control (React Router's Route, Formik's Field, headless UI components) where you want the library to manage behavior while the consumer controls rendering. If you see both patterns available, prefer hooks for logic reuse and render props for rendering flexibility."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Composition fits React's functional model perfectly. Components compose like functions: f(g(x)). Inheritance creates tight coupling and makes refactoring risky. When you pass components as props (slot pattern), they are created by the parent — this matters because their context access and error boundary scope belong to where they're defined, not where they're rendered. Advanced patterns include component injection (`as={CustomButton}`) and polymorphic components (`<Box as=\"section\">`). Typing polymorphic components in TypeScript requires careful generics work, but it's worth it for component library APIs."
      },
