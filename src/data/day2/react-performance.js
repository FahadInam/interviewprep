  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "React.memo isn't free — React still runs the props comparison on every parent render. For very simple components (just rendering a few elements), the comparison overhead can cost more than just re-rendering. When to use it: (1) The component is expensive to render (large lists, charts, heavy calculations). (2) The parent re-renders often but this component's props rarely change. Key gotchas: (1) memo only checks props — context changes bypass it entirely. (2) Passing JSX as children (`<Memo><Child/></Memo>`) breaks memo because children is a new object every render. (3) Inline functions and objects in props break memo — use useCallback/useMemo in the parent. The best optimization is often structural: push state down so fewer components re-render, or lift expensive components above the state that changes."
      },
