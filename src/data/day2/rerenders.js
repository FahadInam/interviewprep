  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "The mental model for re-renders: React re-renders top-down. When a component's state changes, React re-renders it AND all its children by default. The bailout only happens when: (1) props are the exact same reference (===), (2) no pending state updates, (3) context unchanged, and (4) component type didn't change. React.memo adds a shallow props comparison on top of this. The biggest performance lever isn't React.memo — it's pushing state down to where it's needed. If only the search input needs the search text, put the state in the search component, not the parent. This avoids re-rendering siblings entirely, with zero optimization code."
      },
  ...
  return <button className={theme}>Click me</button>;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "The render phase is where React calls your component functions and builds a virtual tree — no DOM touching happens here. In concurrent mode, this phase can be paused and resumed. The commit phase applies actual DOM changes and is always synchronous (can't be interrupted). Even if 100 components re-render, if their output hasn't changed, the commit phase does zero DOM work. This is why re-renders are usually cheap — React's real cost is DOM mutations, which it minimizes automatically. Don't waste time preventing re-renders unless profiling shows they're actually causing slowness. The React DevTools Profiler highlights which commits actually changed the DOM."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "React 18 batches ALL state updates regardless of where they happen — event handlers, setTimeout, Promise callbacks, even native event listeners. Three setState calls in a row = one re-render, one diff, one DOM update. This is a huge performance win. flushSync is the escape hatch: it forces React to process the update and paint immediately, right there in the call stack. Use it when you need to read a DOM measurement right after a state change (like scrolling to a newly added element). But avoid it otherwise — it defeats batching and makes your app slower."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "The children pattern avoids re-renders without any memoization. Here's why: when `<Wrapper>{children}</Wrapper>` is in ParentA, and Wrapper's own state changes, React re-renders Wrapper. But the `children` prop was created by ParentA's render — and ParentA didn't re-render, so `children` is the exact same object reference. React sees same reference = skip that subtree. No React.memo needed. This is why React Router's Provider, Redux's Provider, and most layout components use this pattern. The practical takeaway: instead of wrapping components in React.memo, restructure so the stateful part is a wrapper and the expensive part is passed as children."
      },
