  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "Internally, useEffect creates an effect object on the Fiber: { tag, create, destroy, deps, next }. 'create' is the effect body, 'destroy' is the cleanup, 'deps' is the dependency array. React compares new deps vs old deps using Object.is per element. If any dep changed (or on mount), the effect gets the HookHasEffect flag. After DOM mutations and paint, React flushes passive effects in two passes: first ALL cleanups run, then ALL setups run. This two-pass order matters because both cleanups and setups see the committed DOM. Effects are scheduled async (NormalPriority via scheduler), so they never block the browser paint."
      },
  ...
  // 2. Setup: connect to 'random'
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Comparison happens in areHookInputsEqual: it iterates deps calling Object.is(prevDeps[i], nextDeps[i]). If any returns false, the effect gets the HookHasEffect flag. Key point: objects are compared by reference. [{ id: 1 }] as a dep is always 'changed' because a new object is created each render. Same for arrays and functions. This is why useCallback and useMemo exist -- to keep references stable. Effect execution order: all cleanups across the tree run first (bottom-up), then all setups (top-down, following fiber traversal). The exhaustive-deps rule exists because React cannot see which values are used inside the closure."
      },
  ...
  return <ul>{results.map(r => <li key={r.id}>{r.title}</li>)}</ul>;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Cleanup is stored as 'destroy' on the effect object. During commit, React first iterates all effects with HookHasEffect and calls their destroy functions, then iterates again calling create functions and storing the returned destroy. All cleanups across the tree run before any setups. The cleanup closes over values from its render because it needs to undo that specific sync. For example, cleanup for userId=1 must unsubscribe from userId=1, not the new userId=2. In React 18 Strict Mode, React runs mount -> cleanup -> mount to catch effects that don't clean up properly."
      },
  ...
  return <p>{user.name}</p>;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Commit phase order: (1) Mutation: apply DOM changes. (2) Layout: useLayoutEffect cleanups run, then setups run, refs attach. (3) Browser paints. (4) Passive effects (async): useEffect cleanups run, then setups run. useLayoutEffect fires during step 2 (sync, blocks paint). If useLayoutEffect calls setState, React processes that update sync before paint -- user never sees the middle state. This is why it prevents flicker but must be used sparingly. In SSR, useLayoutEffect warns because there's no DOM to measure."
      },
