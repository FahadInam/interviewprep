  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "useMemo is a performance hint, not a guarantee — React may drop cached values under memory pressure and recompute. So never put side effects in useMemo. The real decision framework: (1) Is the computation actually slow? Profile first — useMemo has its own overhead (storing values, comparing deps). (2) Does the result get passed to a memoized child? Then useMemo prevents unnecessary child re-renders by keeping the same object reference. (3) Are you creating objects/arrays in render that cause useEffect to re-run? useMemo stabilizes the reference. If none of these apply, skip useMemo — the overhead isn't worth it."
      },
  ...
  return <div style={style}>{/* chart here */}</div>;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "useCallback is literally useMemo(() => fn, deps) under the hood. The key insight most developers miss: useCallback alone does nothing for performance. It only helps when paired with a consumer that checks reference equality — React.memo on a child, or as a dependency in useEffect/useMemo. Without that consumer, you're paying the cost of storing and comparing deps for zero benefit. Decision rule: before adding useCallback, ask 'what optimization boundary will consume this stable reference?' If you can't answer that, skip it."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "React.memo does a shallow comparison: it iterates own properties and compares each with Object.is(). Hook dependencies use the same Object.is() comparison on each dep individually. React also has an internal fast path: if a component's props object is the exact same reference as last render, it can skip re-rendering entirely. This is why the 'children as props' pattern works — if the parent doesn't re-render, the children JSX reference stays stable and children skip re-rendering. Understanding this chain (reference equality → bailout decisions) is the foundation of all React performance optimization."
      },
  ...
  return <div>{options.theme}</div>;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Each useMemo/useCallback allocates memory for the cached value and its deps array. Every render, React compares each dep — that's N comparisons per hook per render. For cheap computations, this overhead can exceed the cost of just re-computing. React Compiler (in development) aims to auto-memoize at build time, making manual useMemo/useCallback mostly unnecessary. Until then: profile first, memoize second. Also consider that concurrent features (transitions, Suspense) can reduce the need for manual memoization by keeping the UI responsive during heavy renders — sometimes the better fix is startTransition, not useMemo."
      },
