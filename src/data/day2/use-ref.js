  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "Inside React (mountRef in ReactFiberHooks.js), useRef creates a hook with memoizedState = { current: initialValue }. On updates (updateRef), it just returns the same object — no comparison, no re-render. The ref lives on the fiber's hooks list and is cleaned up on unmount. Refs are set during the commit phase, not render. In concurrent rendering, a component may render many times but ref assignments happen once per commit. This matters for knowing when ref.current is safe to read."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "React sets refs during commitAttachRef (ReactFiberCommitWork.js). For function components, refs attach after all DOM changes in the same commit are done (layout effects phase). For callback refs, the function is called with the DOM node. Refs are NOT available during render, only after commit. In concurrent mode, renders can restart, but ref assignments happen once per final commit. Callback refs (ref={node => ...}) let you react to attachment and detachment. They are the only way to get notified when a ref changes."
      },
  ...
  return <span>Renders: {count.current}</span>;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "forwardRef creates an element with REACT_FORWARD_REF_TYPE. React calls the inner function with (props, ref) instead of just (props). useImperativeHandle uses useLayoutEffect internally to assign the handle during commit. Since React 19, forwardRef is not needed — ref is a regular prop. useImperativeHandle stores a factory and deps; during commitAttachRef, it calls the factory and assigns the result to the parent's ref.current. Component libraries use this pattern to expose clean APIs (e.g., Dialog with open/close methods)."
      },
