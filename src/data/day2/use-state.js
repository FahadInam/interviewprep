  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "At mount, mountState creates a hook via mountWorkInProgressHook(), sets hook.memoizedState = initialState (calling it if it's a function), creates an update queue, and binds dispatchSetState to the fiber and queue. The dispatch creates an update object { lane, action, next }, adds it to the circular linked list queue.pending, and calls scheduleUpdateOnFiber. On re-render, updateState delegates to updateReducer since useState is useReducer with basicStateReducer: (state, action) => typeof action === 'function' ? action(state) : action. It processes queued updates in order. There is also an eager bailout: if dispatch is called outside render with an empty queue, React computes the next state and compares with Object.is. If equal, it skips scheduling entirely."
      },
  ...
  return <div>{name}, {age}, {active.toString()}</div>;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "During mountState, React checks if initialState is a function. If so, it calls it: initialState = initialState(). This only happens at mount. During updateState (re-renders), initialState is never accessed — React reads from hook.memoizedState. The savings matter when the initializer reads localStorage, parses JSON, or computes derived data. Note: the lazy initializer gets no arguments. In Strict Mode, React calls it twice to detect side effects but only uses the first result."
      },
  ...
  return <Grid data={grid} onChange={setGrid} />;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Under the hood, useState is useReducer with basicStateReducer: (state, action) => typeof action === 'function' ? action(state) : action. When React processes the update queue, direct values replace the state. Functional updaters are called with the accumulated state: newState = update.action(newState). The queue processes in order, so three setState(prev => prev + 1) calls chain: 0 -> 1 -> 2 -> 3. There is also an eager bailout: for both direct values and updaters, if the queue is empty, React computes the state early and compares with Object.is to skip a re-render if unchanged."
      },
  ...
  return <button onClick={handleClick}>{count}</button>;
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "The bailout check runs in two places. First, in dispatchSetState (eager bailout): if the queue is empty and the fiber has no pending work, React computes the new state early and compares via Object.is. If equal, no re-render is scheduled at all. Second, during updateReducer in the render phase: after processing all queued updates, React compares the result with the old memoizedState. If equal, it sets didReceiveUpdate = false and can skip children. Note: even with a bailout, React may still render the component itself (but not children) if there were pending updates in the queue."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Batching works through the lane model and scheduler. In React 18, all setState calls enqueue updates and call ensureRootIsScheduled, which avoids scheduling duplicate tasks. The render is scheduled as a microtask or callback, so all synchronous setState calls in the same stack batch naturally. In React 17, an executionContext flag controlled this — set inside React code (batched), unset outside (each setState flushed immediately). React 18 removes this split. flushSync works by calling flushSyncCallbacks to force a synchronous render and commit before returning."
      },
