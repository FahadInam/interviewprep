  ...
  // React collects all changes, diffs once, updates DOM once
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Diffing operates on Fiber nodes during the render phase. For each Fiber, React compares the new element's type and key against the current Fiber. Single element: matching types clone the Fiber with updated props (UPDATE flag); different types mark old for DELETION and create new Fiber (PLACEMENT). Lists (reconcileChildFibers): builds a map of existing children by key, iterates new children to find matches, flags unmatched old items for DELETION. A lastPlacedIndex tracker determines if matched nodes need DOM reordering — a node moves only if its old index < lastPlacedIndex. This means [A,B,C,D] to [D,A,B,C] moves A,B,C (not D)."
      },
...
// Result: 3 DOM changes instead of 1! Keys fix this.`
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "In reconcileChildFibers, React builds an existingChildren Map<key|index, Fiber>. It iterates new elements, looks up each by key, and on match calls useFiber to clone with updated props. Remaining map entries get DELETION flags. The lastPlacedIndex optimization tracks the rightmost reused index — matched Fibers with old index >= lastPlacedIndex stay in place; otherwise they get PLACEMENT (move) flags. This makes appends fast but prepends slow. Unstable keys (Math.random()) cause full unmount/remount every render, destroying all state."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "React 18 batching uses the Fiber scheduler's lane model. Each setState enqueues an update on the Fiber's updateQueue at an appropriate priority lane. Updates in the same event or microtask share a lane and process in one render pass. The scheduler uses MessageChannel (not setTimeout) to avoid the 4ms delay clamp. flushSync forces immediate processing. In concurrent mode, React can pause a render to handle higher-priority updates first. Before React 18, only synthetic events and lifecycle methods were batched."
      },
