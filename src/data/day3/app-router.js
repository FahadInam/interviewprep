  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "The App Router shifts from page-level to segment-level architecture. Each route segment manages its own data, loading, and error states independently. It uses React concurrent features: Suspense for streaming, transitions for navigations, and a lightweight server-rendered payload instead of full HTML for client navigations. Only changed segments re-render on navigation; shared layouts stay in memory. The client-side router cache stores rendered segments for instant back/forward. This means fewer waterfalls, smaller JS bundles, and granular per-segment caching. Edge runtime is first-class -- layouts and pages can independently choose edge or Node.js runtime."
      },
