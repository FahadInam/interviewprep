  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "The key thing to understand is that Next.js has multiple cache layers that interact in non-obvious ways. The Data Cache stores individual fetch responses and persists across deployments on platforms like Vercel. The Full Route Cache stores the pre-rendered page output. The Router Cache is client-side. These are independent. When you set { cache: 'no-store' } on one fetch, you opt that request out of the Data Cache, but the bigger consequence is that a single no-store fetch makes the entire route dynamic -- no static generation possible. In production, stale data bugs usually come from not understanding which cache layer is serving old content. You might update the database but still see old data because the Data Cache entry was never invalidated. The mental model I use: think of caching as layered -- each layer can serve stale content independently, and you need to invalidate the right one."
      },
  ...
  next: { tags: ["posts"] },
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "The critical distinction: Next.js automatically deduplicates identical fetch() calls, but database queries through an ORM like Prisma don't go through fetch. That's where React cache() comes in -- it deduplicates any async function, not just fetch. It only works within a single render pass (one user request), not across requests. The practical pattern I'd recommend: export a cached data-fetching function from a shared module (e.g., lib/data.js) that multiple Server Components import. Each component calls getUser() independently -- no prop drilling needed -- and cache() ensures the actual database query only runs once per request. One gotcha: cache() compares arguments by reference, so passing a new object like { id: 1 } each time won't deduplicate because each object is a different reference. Stick to primitive arguments (strings, numbers) for reliable deduplication."
      },
