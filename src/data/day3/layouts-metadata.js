...
// keeps the Sidebar and DashboardHeader mounted — only {children} changes`
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "The key difference is how React treats them during navigation. Layouts are treated as the same component instance across navigations -- React keeps them mounted and preserves their state. Templates get a fresh instance on every navigation -- React unmounts the old one and mounts a new one. This is a deliberate design choice with practical implications: (1) Enter/exit animations -- template remounts trigger CSS/Framer Motion enter animations on every page change. (2) Page-view analytics -- useEffect in a template runs on every navigation, giving you automatic page tracking. (3) State isolation -- form state in a template won't leak between pages. (4) Performance trade-off -- templates add overhead because they unmount/remount the entire subtree, including all child components. Use them sparingly and only when you genuinely need the remounting behavior."
      },
