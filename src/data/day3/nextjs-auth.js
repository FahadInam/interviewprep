  ...
  concepts: [
    {
      ...
      explanations: {
        layman: `Think of going to an amusement park. At the entrance, you show your ticket and they stamp your hand. After that, every ride just checks for the stamp -- you never show your ticket again. That hand stamp is a cookie.\n\nCookie-based auth works the same way: you log in once (show your ticket), the server stamps your browser with a cookie, and every future request automatically includes that stamp. The server sees the stamp and knows who you are without asking again. The stamp is invisible to you (HttpOnly) and only works at this park (SameSite).`,
        mid: `Cookie-based authentication stores a session identifier in an HTTP cookie. The flow is:\n\n1. User submits credentials (username/password)\n2. Server validates credentials against the database\n3. Server creates a session record (in DB, Redis, or memory)\n4. Server sets a Set-Cookie header with the session ID\n5. Browser automatically includes this cookie in subsequent requests\n6. Server reads the cookie, looks up the session, identifies the user\n\nIn Next.js App Router, you use the cookies() function from 'next/headers' to read and set cookies in Server Components and Server Actions. The cookie should have HttpOnly, Secure, SameSite=Lax, and a reasonable max-age.`,
        senior: `Cookie-based session auth in Next.js App Router requires careful architectural decisions:\n\n1. Session Storage: In-memory sessions don't survive server restarts and don't work in serverless. Use Redis, a database, or encrypted stateless cookies (like iron-session or Jose JWE). In Vercel deployments, serverless functions are stateless, so external session stores are mandatory.\n\n2. cookies() is a dynamic API in Next.js — using it in a Server Component opts that route out of static rendering. This is by design since auth-gated pages must be dynamic.\n\n3. Cookie configuration must account for the deployment environment: domain attribute for subdomains, path for route-specific cookies, __Secure- or __Host- prefixes for enhanced security.\n\n4. Session rotation: After login, always create a new session ID to prevent session fixation attacks. After sensitive operations (password change), invalidate all other sessions.\n\n5. CSRF protection: SameSite=Lax covers most cases, but for APIs consumed by third-party forms, you need explicit CSRF tokens. Server Actions in Next.js have built-in origin checking.`
      },
...
export default async function DashboardPage() {
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        layman: `A cookie is like a hand stamp -- the park keeps a record of you and just checks the stamp. A JWT is different: instead of a stamp, the park gives you a sealed ID card with your name, photo, and membership level printed on it, sealed with a tamper-proof hologram.\n\nYou carry this card yourself and show it at every ride. The ride operator does not need to call the front office to verify you -- they just check the hologram is legit and read your info right off the card. The trade-off: if you lose the card, there is no way to cancel it until it expires. Cookies can be revoked instantly. JWTs cannot.`,
