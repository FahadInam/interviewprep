  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "The Server/Client boundary is a module-level decision, not a runtime one. When the bundler sees 'use client', it draws a line: everything above stays server-only, everything at and below gets included in the client bundle. Server Components are never in the client bundle -- they're replaced by their pre-rendered output. This is why Server Components can safely contain secrets like API keys and database passwords -- that code never reaches the browser. In production, this architecture dramatically reduces bundle size: a page with 50 components where only 3 need interactivity ships JavaScript for only those 3. The server tree can be arbitrarily deep and complex without any client-side cost. The key constraints to internalize: data crossing the boundary must be serializable (no functions, classes, or Dates as props), and the boundary is one-directional -- server renders, client hydrates, never the reverse. The decision of where to place this boundary is one of the most impactful architectural choices in a Next.js app."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "The 'use client' directive is a bundler instruction that creates a split point in your module graph. Everything at and below that directive gets included in the client JavaScript bundle. The boundary is transitive -- if a 'use client' file imports other files, those get pulled into the client bundle too. This has a huge practical implication for bundle size: placing 'use client' on a barrel file (index.js that re-exports 20 components) pulls ALL 20 into the client bundle, even if only 1 is used. The rule of thumb: place 'use client' on the leaf components that actually need interactivity, not on shared entry points. Props passed across this boundary must be serializable (JSON-compatible) -- no functions, class instances, or non-serializable objects. In production, regularly analyze your client bundle to catch accidental boundary crossings. A Server Component that imports a utility which happens to import a 'use client' module can silently pull server-only code into the client."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "This pattern works because of a simple but important distinction: passing vs importing. You CAN'T import a Server Component inside a Client Component -- the Client Component would need to execute it, but Server Components can only run on the server. You CAN pass Server Components as children because the parent Server Component renders them first on the server, and the Client Component just receives the pre-rendered output -- it never needs to execute the Server Component itself. Think of it like a picture frame: the frame (Client Component) doesn't need to know how to paint the picture (Server Component) -- it just displays what's already been painted. This pattern is essential for: animated wrappers around server content, stateful containers (tabs, accordions) with server-rendered panels, context providers that need to wrap server content, and modal/dialog patterns where the trigger is client-side but the content is server-rendered. Master this pattern -- it's the key to keeping your bundles small while still having rich interactivity."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "The serialization boundary creates real constraints you'll hit in production. The practical implications for large apps: (1) Database ORM objects (Prisma models, Mongoose documents) often contain methods and getters that aren't serializable -- map them to plain objects before passing to Client Components. (2) Date objects are supported but can behave inconsistently -- consider serializing to ISO strings explicitly. (3) React elements (JSX) can be passed as props -- this is how the children composition pattern works. (4) Functions trigger serialization errors, so event handlers must be defined inside Client Components. For third-party libraries, any library that uses hooks, context, browser APIs, or React state must be used in Client Components. If a library doesn't have a 'use client' directive, you need to create a wrapper component that adds it. Many popular libraries (Recharts, Framer Motion, React Hook Form) are client-only and need these wrappers. The community convention is moving toward libraries exporting separate server and client entry points -- always check if a library supports Server Components before wrapping it."
      },
