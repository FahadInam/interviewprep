  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "In the App Router, SSR is the fallback rendering mode triggered by dynamic data dependencies. The framework detects dynamic usage at the segment level -- if any component in a segment uses cookies(), headers(), or uncached fetch, that entire segment becomes dynamic. The server generates HTML for the initial paint plus the data React needs to hydrate and make the page interactive. With streaming SSR, the layout shell streams immediately while dynamic segments stream in as their data resolves. This is a fundamental improvement over Pages Router SSR where the entire page blocked until getServerSideProps completed. In production, SSR pages can run on Edge Runtime (export const runtime = 'edge') for lower latency, but Edge has limitations (no Node.js APIs like fs, limited memory). The time to first byte for SSR depends on your data source latency -- always co-locate your server and database. SSR also enables personalization at the HTML level, which is critical for pages that vary by user but still need SEO."
      },
  ...
  return (
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "Streaming SSR fundamentally changes the performance profile of server-rendered pages. Instead of waiting for all data to resolve before sending any HTML, the server progressively sends HTML as components finish rendering. When a Suspense boundary wraps an async component, the server sends the fallback content immediately and continues rendering other parts of the page. When the suspended component resolves, the real content replaces the fallback -- users see content appear progressively. This means the critical rendering path only includes the shell -- slow data sources don't block the initial paint. In production, streaming pairs well with Edge Runtime for minimal time to first byte. The key architectural decision is where to place Suspense boundaries. Too many creates visual jank (many things popping in at different times). Too few defeats the purpose (large sections still block). The sweet spot: place Suspense boundaries at data-dependency boundaries -- each section that fetches from a different data source gets its own boundary. This way, fast APIs render immediately while slow ones stream in later."
      },
