  ...
  return [value, setCookie, removeCookie];
    ...
    },
    {
      ...
      explanations: {
        ...
        senior: "SRI is a critical defense against supply chain attacks — one of the fastest-growing attack vectors. The 2018 event-stream incident (malicious code injected into an npm package) and the 2021 ua-parser-js compromise demonstrate real-world supply chain risks. For CDN-hosted resources, SRI provides integrity verification — the browser computes a hash of the downloaded file and compares it to the hash you declared in your HTML. If they don't match, the file is blocked. Implementation strategy: (1) Automate hash generation in your build pipeline — most bundlers have SRI plugins, and CI can generate hashes as part of the asset pipeline. (2) SHA-384 is the standard algorithm — good balance of security and performance. (3) Always pair with `crossorigin='anonymous'` for cross-origin resources so the browser can actually verify them. (4) Plan a fallback strategy: if the CDN file fails integrity check, load from your own origin instead. (5) The key trade-off is that SRI doesn't work for resources that change without versioned URLs (like auto-updating analytics scripts), and you need to update hashes whenever you update the resource version. For modern SPAs using bundlers, self-hosting dependencies eliminates the need for SRI on most resources, but it remains essential for any externally hosted scripts. The bigger architectural decision is whether to self-host vs CDN-host at all — self-hosting gives you full control and eliminates SRI overhead, while CDN-hosting provides better geographic distribution but requires SRI for security."
      },
