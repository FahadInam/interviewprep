  ...
  concepts: [
    {
      ...
      explanations: {
        ...
        senior: "In React/Next.js apps, here's what to watch for in code reviews:\n\n• `dangerouslySetInnerHTML` without DOMPurify — always sanitize first\n• Dynamic `href` built from user input — check for `javascript:` protocol: `if (url.startsWith('javascript:')) reject`\n• `postMessage` handlers without origin checking — always verify `event.origin`\n• `ref.current.innerHTML = userInput` — bypasses React's escaping entirely\n• SVG uploads — SVGs can contain `<script>` tags, sanitize before displaying\n\nReact escapes JSX by default, which prevents most XSS. But the escape hatches above are real attack vectors. In SPAs, DOM-based XSS is the biggest risk because the attack happens entirely in the browser — reading from `location.hash` or URL params and inserting into the DOM without sanitization. Defense-in-depth: use CSP headers (`Content-Security-Policy`) to block inline scripts even if XSS gets through your code, and add `HttpOnly` on auth cookies so stolen sessions aren't possible."
      },
